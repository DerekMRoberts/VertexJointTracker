...
// DESCRIPTION:
// 
// Produces the MEL command "idleTest".
//
// The idleTest plug-in shows an example of using both the idle messages and UI deleted messages in a simple plug-in.
// These messages correspond to the -idleEvent and -uiDeleted flags for the scriptJob command.
//
// When you load the plug-in, it adds the command "idleTest". To run it, type "idleTest n" where n is some positive number.
// Idle test will then create a window and start filling it with a list of prime numbers.
// The test will compute one new prime number for each idle message that it receives.
// You will notice that idle messages stop during playback or while dragging an object.
//
// If you type a large enough number for n, you will also notice that the idle messages will use up *all* available CPU cycles.
// For this reason, plug-ins need to cancel their requests for idle messages once they are no longer needed.
//
