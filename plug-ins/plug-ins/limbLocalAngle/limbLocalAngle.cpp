...
DeclareSimpleCommand(armLocalAngle, "Autodesk", "1.0")

MStatus armLocalAngle::doIt(const MArgList &)
{
    MDagPath node;
    MObject component;
    ...
    for(unsigned int i = 0; i < locGroup.length(); i++)
    {
        if (!locations.empty()) {
            ...
            if((i+1)%3 == 0 && i > 0 && (i+1)%3 <= locGroup.length())
            {
                ...
                cout << "Distance between Locator " << i-1 << " and Locator " << i+1 << ": " <<  distance << " cm\n";
                auto theta = acos((pow(distances[i-2],2) + pow(distances[i-1], 2) - pow(distances[i],2))/ (2 * distances[i-2] * distances[i-1])) * 180/3.1415;
            ...
            if(i+1 < locGroup.length() && (i+1)%3 != 0)
            {
                ...
                cout << "Distance between Locator " << i+1 << " and Locator " << i+2 << ": "<< distance << " cm\n";
            }
        }
    }
    return MS::kSuccess;
